...
from src.environment.fly_spatial_parameters_multi_video import FlySpatialParameters
...
class FlyNavigator(Env):
	...
	def __init__(self, rng, config):
		...
		self.dt = None #to be initialized in reset method
		## Define odor plume parameters

		plume_dict_list = plume_dict['PLUME_DICT_LIST']
		self.plume_dict_list = plume_dict_list		
		self.plume_list = []

		for i, plume_config in enumerate(plume_dict_list):
			if plume_config['PLUME_TYPE'] == 'movie':
				test_config = {'plume':plume_config} #just for backwards compatibility for now
				self.plume_list.append(OdorPlumeFromMovie(test_config))

			elif plume_config['PLUME_TYPE'] == 'ribbon':
				test_config = {'plume':plume_config} #just for backwards compatibility for now
				self.plume_list.append(StaticGaussianRibbon(test_config))
		...
		self.source_location = None #initialized in reset method
		self.mm_per_px = None #initialized in reset method
		self.conc_threshold = None #initialized in reset method

		## Define reset parameters
		self.min_reset_x = agent_dict['MIN_RESET_X_MM']
		self.max_reset_x = agent_dict['MAX_RESET_X_MM']
		self.min_reset_y = agent_dict['MIN_RESET_Y_MM']
		self.max_reset_y = agent_dict['MAX_RESET_Y_MM']
		...
		self.theta_random_bounds = np.array([agent_dict['INIT_THETA_MIN'], agent_dict['INIT_THETA_MAX']])
		self.initial_max_reset_x = agent_dict['INITIAL_MAX_RESET_X_MM']
		self.reset_x_shift = agent_dict['RESET_X_SHIFT_MM']
		self.x_random_bounds = np.array([self.min_reset_x, self.initial_max_reset_x])
		self.y_random_bounds = np.array([self.min_reset_y, self.max_reset_y])
		self.shift_episodes = agent_dict['SHIFT_EPISODES']
		...
		#self.fly_trajectory = np.zeros((self.max_frames, 2)) + np.nan #only defining this in reset method now
		self.all_plume_inds = []
	...
	def _init_plume_variables(self):

		current_plume_dict = self.plume_dict_list[self.plume_ind]
		self.odor_features.max_conc = current_plume_dict['MAX_CONCENTRATION']
		self.odor_features.base_threshold = current_plume_dict['CONCENTRATION_BASE_THRESHOLD']
		self.odor_features.dt = current_plume_dict['DELTA_T_S']
		self.dt = current_plume_dict['DELTA_T_S']
		self.mm_per_px = current_plume_dict['MM_PER_PX']
		self.odor_features.mm_per_px = current_plume_dict['MM_PER_PX']
		self.conc_threshold = current_plume_dict['CONCENTRATION_BASE_THRESHOLD']
		...
		#setting odor_features std_box and num_pts
		...
		self.source_location = current_plume_dict['SOURCE_LOCATION_MM']
