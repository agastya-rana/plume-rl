...
class packets:
    ...
    def __init__(self, rate = 10, dw_speed = 150, init_intensity = 3827.24, cw_type = 'Gaussian', eddy_D = 0.1*(10**4), 
    	      ...
    	      rand_gen = np.random.RandomState(0), signal_noise = False, noise_std = 0.1, corr_lambda = None, cw_speed = None):
    ...
    def generate_packets(self, delta_t, rand_gen): #generates and evolves packets by one time step

        rand_unif_1 = rand_gen.random_sample(1)
        ...
        self.packet_xs = delta_t*self.dw_speed + self.packet_xs #advection downwind due to mean flow
        ...
        if self.cw_type == 'Gaussian':
            ...
            flip_rands = rand_gen.uniform(size = len(self.packet_signs))

            flip_bool = flip_rands < delta_t*self.corr_lambda

            self.packet_signs[flip_bool] = -1*self.packet_signs[flip_bool]

            perts = np.zeros((len(self.packet_signs),2))

            perts[:,0] = self.cw_speed*self.packet_signs*delta_t


        elif self.cw_type == 'O-U':

            perts = np.zeros((len(self.packet_xs),2))
            du_y = -self.packet_vel_y*delta_t*self.corr_lambda + np.sqrt(2*self.cw_speed*delta_t*self.corr_lambda)*rand_gen.normal(loc = 0, scale = 1, size = len(self.packet_ys))
            self.packet_vel_y = self.packet_vel_y + du_y

            perts[:,0] = self.packet_vel_y*delta_t
