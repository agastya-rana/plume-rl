...
def detect_local_odor_motion(fly_location: np.ndarray, odor_frame_1: np.ndarray, odor_frame_2: np.ndarray) -> list:
    x, y = np.floor(fly_location).astype(int)
    ...
    # for element in odor_frame_1:
    #     if element.any() != 1:
    #         for element_2 in element:
    #             if element_2.any() != 1:
    #                 print("this is the element_2 within odor_frame_1 that is not equal to 1", element_2)
    ...
    local_odor_motion = local_odor_motion_x, local_odor_motion_y
...
class OdorFeatures:
    ...
    motion_speed: list = field(default_factory=list)
    ...
    def discretize_features(self):  # This should be factored somewhere else
        ...
        if self.motion_speed[0] == 0:
            motion_speed_hack_attempt[0] = 0
        elif self.motion_speed[0] > 0:
            motion_speed_hack_attempt[0] = 1
        else:
            motion_speed_hack_attempt[0] = 2

        if self.motion_speed[1] == 0:
            motion_speed_hack_attempt[1] = 0
        elif self.motion_speed[1] > 0:
            motion_speed_hack_attempt[1] = 1
        else:
            motion_speed_hack_attempt[1] = 2

        return np.array([concentration, gradient_hack_attempt[0], gradient_hack_attempt[1], motion_speed_hack_attempt[0], motion_speed_hack_attempt[1]])
