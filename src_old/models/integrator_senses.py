...
from src.models.geometry import standardize_angle, AngleField
...
class IntegratorSensor:
    ...
    def update(self, homing_vector: np.ndarray):
        if np.array_equal(np.array([0, 0]), homing_vector):
            ...
            self.home_angle = standardize_angle(np.arctan2(homing_vector[1], homing_vector[0]))
    ...
    def discretize_angle(self, angle_bin_number: int = ANGLE_BIN_NUMBER) -> int:
        angle_bins = np.linspace(0, 2 * np.pi, angle_bin_number)
        return np.digitize(self.home_angle, angle_bins, right=False).item()
