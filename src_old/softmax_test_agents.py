...
config_dict['WALK_SPEED_MM_PER_S'] = 60
config_dict['ANTENNA_LENGTH_MM'] = 2.8
...
q_table_path = '../trained_models/full_model_all_good_Q.npy'
...
for episode in range(config_dict['N_EPISODES']):
    ...
    while not done: # Advance the environment (e.g., the smoke plume updates and the agent walks a step)
        ...
        if explore:
            ...
            vals = q_table[tuple(observation)] 
            probs = np.exp(vals/T)/(np.sum(np.exp(vals/T)))
            action_inds = np.arange(len(vals))
            action = environment.rng.choice(action_inds, p = probs, size=1).astype(int)
